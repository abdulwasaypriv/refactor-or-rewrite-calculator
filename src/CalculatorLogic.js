/**
 * CalculatorLogic.js
 * Rewrite vs. Refactor Decision Engine
 * Weighted scoring system — Tech Debt & Timeline carry highest influence.
 */
...
/**
 * @param {number} cost      1–10 (1 = low cost, 10 = extremely expensive)
 * @param {number} effort    1–10 (1 = trivial, 10 = enormous lift)
 * @param {number} timeline  1–10 (1 = ample time, 10 = critical deadline)
 * @param {number} teamSize  1–10 (1 = solo, 10 = large org)
 * @param {number} techDebt  1–10 (1 = clean codebase, 10 = unmaintainable)
 * @returns {{ recommendation: string, riskScore: number, justification: string }}
 */
export function calculateStrategy({ cost, effort, timeline, teamSize, techDebt }) {
  const inputs = { cost, effort, timeline, teamSize, techDebt };

  for (const [key, val] of Object.entries(inputs)) {
    if (!Number.isFinite(val) || val < 1 || val > 10) {
      throw new RangeError(`Input "${key}" must be a finite number between 1 and 10.`);
    }
  }
